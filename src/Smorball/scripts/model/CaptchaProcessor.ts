...
class CaptchaProcessor {
    ...
    processCaptchaData(data) {
        ...
        this.config.loader.load([{ src: myData.url + ".jpeg", id: myData._id }], () => {
            this.config.gameState.captchaDatasArray.push(myData);
            if ((this.captchaDatas == undefined || this.captchaDatas.local) && this.config.gameState.currentLevel != 1) {
                this.activateCaptchaSet();
            }
        });
